...
def addLinkElements(filename):
    ...
    with open(filename, "w") as output:
        for line in contents:
            ...
            if searchObj:
                text = re.sub(r"\s*<a(.+)", "", searchObj.group(2))
                id = text.replace(" ", "-").lower()
                # output.write(getLinkElement(id) + "\n")
                output.write(searchObj.group(1) + ' ' + text + ' ' +
                             getLinkElement(id) + "\n")
            else:
                output.write(line)
